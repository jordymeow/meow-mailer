...
if ( !class_exists( 'MeowKit_MWMAIL_Admin' ) ) {

  class MeowKit_MWMAIL_Admin {
    ...
    public function __construct( $prefix, $mainfile, $domain, $isPro = false, $disableReview = false, $freeOnly = false ) {
      ...
      if ( is_admin() ) {
        ...
        if ( MeowKit_MWMAIL_Helpers::is_asynchronous_request() ) {
          return;
        }
    ...
    public static function process_deferred_instances() {
      ...
      if ( !function_exists( 'current_user_can' ) || !function_exists( 'wp_get_current_user' ) ) {
        trigger_error(
          'MeowKit_MWMAIL_Admin: Pluggable functions still unavailable on init hook. ' .
    ...
    private function run_admin_setup() {
      ...
      if ( !MeowKit_MWMAIL_Admin::$loaded ) {
        // Check for potential issues with WordPress install, other plugins, etc.
        new MeowKit_MWMAIL_Issues( $this->prefix, $this->mainfile, $this->domain );
        ...
        MeowKit_MWMAIL_Admin::$loaded = true;
      }
      ...
      if ( $this->is_user_admin() ) {
        if ( !$this->disableReview ) {
          new MeowKit_MWMAIL_Ratings( $this->prefix, $this->mainfile, $this->domain );
        }
        new MeowKit_MWMAIL_News( $this->domain );
      }
    }
    ...
    public function admin_menu_start() {
      ...
      if ( !isset( $submenu[ 'meowapps-main-menu' ] ) ) {
        add_menu_page(
          'Meow Apps',
          '<img alt="Meow Apps" class="meowapps-menu-icon" src="' . MeowKit_MWMAIL_Admin::$logo . '" />Meow Apps',
          'manage_options',
          'meowapps-main-menu',
    ...
    public function meowapps_hide_dashboard_callback() {
      ...
      echo MeowKit_MWMAIL_Helpers::wp_kses( $html );
    }
